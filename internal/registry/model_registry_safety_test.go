...
func TestLookupModelInfoIncludesClaudeFable51(t *testing.T) {
	...
	if model.Thinking == nil || !model.Thinking.AlwaysOn || !model.Thinking.DynamicAllowed || model.Thinking.ZeroAllowed || model.Thinking.Min != 0 || model.Thinking.Max != 0 {
		t.Fatalf("expected Claude Fable 5.1 always-on adaptive thinking, got %+v", model.Thinking)
	}
