...
func TestDetectClaudeCodeRequestAcceptsConfiguredMeasuredBaseline(t *testing.T) {
	...
	if detection := DetectClaudeCodeRequest(headers, payload, false); !detection.Confirmed {
		t.Fatalf("default detection = %#v, want newer 2.2.0 confirmed", detection)
	}
...
func TestDetectClaudeCodeRequestRejectsMalformedNativeSignals(t *testing.T) {
	tests := []struct {
		...
		{name: "below-baseline user agent", headers: http.Header{"User-Agent": {"claude-cli/2.1.257 (external, cli)"}, "X-App": {"cli"}, "Anthropic-Beta": {"claude-code-20250219"}}, userID: validClaudeCodeMetadataUserID},
